...
fn refine_input(input: InputData) -> Vec<Vec<i32>> {
    let reports: Vec<Vec<i32>> = input.list.iter().map(
        |line| line.iter().map(
            |level| level.parse().expect("Failed to parse level")
        ).collect()
    ).collect();
...
fn check_report(report: &Vec<i32>) -> (bool, Vec<usize>) {
    ...
    (0..report.len()-1).for_each(|x| {
        ...
        // Check if it is ascending or descending currently.
        if diff > 0 { descending = true; } 
        else if diff < 0 { ascending = true; };

        // Mark the error location for later.
        if ascending == true && descending == true { 
            errors.push(x);
            errors.push(x+1);
            if x == 1 { errors.push(0); }
        };
    ...
    match (ascending, descending, in_range) {
        // Both ascended and descended, unsafe.
        (true, true, _) => (false, errors),
        // Neither ascended or descended, unsafe.
        (false, false, _) => (false, errors),
        // Differences outside of acceptable range, unsafe.
        (_, _, false) => (false, errors),
    ...
    for report in reports {
        ...
        if result.0 == true {
            ...
            for x in result.1.iter() {
                ...
                if check_report(&unsafe_report).0 == true {
                    safe_2 += 1;
                    break;
                }
            }
