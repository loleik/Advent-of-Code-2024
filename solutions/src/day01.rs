...
fn part1(lists: &(Vec<i32>, Vec<i32>), cumulative_distance: i32, index: usize) -> i32 {
    ...
    let current: i32 = cmp::max(lists.0[index], lists.1[index]) - cmp::min(lists.0[index], lists.1[index]);
...
fn count_repetition(target: i32, list: &Vec<i32>) -> i32 {
    let mut counter: i32 = 0;

    for x in list.iter() {
        if *x == target { counter += 1 }
    }
